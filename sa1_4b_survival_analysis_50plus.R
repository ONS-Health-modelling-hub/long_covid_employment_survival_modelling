...
### restrict to participants aged 50+ at index date
orig <- orig[orig$age_index>=50,]

### create empty list to store overlay plots
plot_list_all_outcomes_50plus <- as.list(NULL)
...
for(i in 1:length(outcomes)) {
  ...
  ggsave(filename=paste0(out_dir, "/ci_unwtd_", outcome, "_50plus.jpg"),
  ...
  ggsave(filename=paste0(out_dir, "/ci_wtd_", outcome, "_50plus.jpg"),
         width=12, height=10, units="cm")
  ...
  write.csv(mod_out,
            file=paste0(out_dir, "/model_output_", outcome, "_50plus.csv"),
            row.names=FALSE)
  ...
  ggsave(filename=paste0(out_dir, "/parametric_fits_", outcome, "_50plus.jpg"),
         plot=comb_plot,
  ...
  ggsave(filename=paste0(out_dir, "/overlay_wtd_", outcome, "_50plus.jpg"),
         plot=print(overlay_plot),
         width=12, height=10, units="cm")
  
  ### store plot in list
  plot_list_all_outcomes_50plus[[i]] <- overlay_plot
  
}

### write out plot list
save(plot_list_all_outcomes_50plus, file=paste0(out_dir, "/plot_list_all_outcomes_50plus.RData"))

...
### get person-level variables at index date
dat_index <- dat %>%
  filter(visit_date==index_date) %>%
  select(participant_id, long_covid_have_symptoms, age_at_visit, age10, sex,
         ethnicityg, white, imd_quintile, gor9d, country, health_conditions,
         health_conditions_impact, health_status, work_status, work_sector, soc_major,
  ...
  rename(lc_index = long_covid_have_symptoms, age_index = age_at_visit, age10_index = age10,
         ...
         imd_quintile_index = imd_quintile, gor9d_index = gor9d,
         country_index = country, health_conditions_index = health_conditions,
         health_conditions_impact_index = health_conditions_impact,
         health_status_index = health_status, work_status_index = work_status,
         work_sector_index = work_sector, soc_major_index = soc_major,
         self_employed_index = self_employed,
         healthsocialcare_pf_index = healthsocialcare_pf,
         remote_collection_index = remote_collection)

dat <- dat %>%
  left_join(dat_index, by=join_by(participant_id==participant_id))
...
### find date of first positive LC response among non-exposed group
dat <- sqldf("
  select a.*, b.first_lc_censor_date
  from dat as a
  left join(
    select participant_id, min(visit_date) as first_lc_censor_date
    from dat
    where lc_any = 1 & lc_index = 0
    group by participant_id
  ) as b
  on a.participant_id = b.participant_id
")
...
### change in sector: censor at retirement, becoming a student, turning 65, last follow-up visit, or reporting LC
dat$fudate_change_sector <- pmin(dat$first_change_sector_date,
                                 ...
                                 dat$last_visit_date,
                                 dat$first_lc_censor_date,
                                 na.rm=TRUE)

### change in SOC group: censor at retirement, becoming a student, turning 65, last follow-up visit, or reporting LC
...
### retirement: censor at becoming a student, turning 65, last follow-up visit, or reporting LC

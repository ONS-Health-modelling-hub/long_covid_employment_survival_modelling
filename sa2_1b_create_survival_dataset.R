...
### restrict dataset to exposed participants, and those in the non-exposed group who never
### reported LC after index date
dat <- sqldf("
  select a.*, b.lc_ever
  from dat as a
  left join(
    select participant_id, max(lc_any) as lc_ever
    from dat
...
dat <- dat[dat$lc_index==1 | (dat$lc_index==0 & dat$lc_ever==0),]
